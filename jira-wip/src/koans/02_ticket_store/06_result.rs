...
impl TicketDraft {
    ...
    pub fn new(title: String, description: String) -> Result<TicketDraft, ValidationError> {
        ...
        if title.len() > 50 {
            return Err(ValidationError("Title cannot be longer than 50 chars!".to_string(),))
        }
        if description.len() > 3000 {
            return Err(ValidationError("Description cannot be longer than 3000 chars!".to_string(),))
        }
